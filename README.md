...
- Credentials are stored in the desktop's Secret Service (gnome-keyring,
  KWallet, ...)
...
- `secret-tool` (part of `libsecret`) for keyring-backed password storage.
  Without it, passwords fall back to being stored in the plugin's config
  file directly.
